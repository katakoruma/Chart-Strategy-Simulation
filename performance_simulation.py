...
class PerformanceAnalyzer:
    ...
    def random_swing_trade(self, 
        ...
        self.random_swing_performance_analyse, self.random_swing_transaction_cost, self.random_swing_tax = self._compute_performance(data, trade_dates, trade_cost, spread, saving_plan, saving_plan_period, tax_rate, tax_allowance)
    ...
    def swing_trade(self, 
        ...
        self.swing_performance_analyse, self.swing_transaction_cost, self.swing_tax = self._compute_performance(data, trade_dates, trade_cost, spread, saving_plan, saving_plan_period, tax_rate, tax_allowance)
    ...
    def buy_and_hold(self, 
        ...
        self.buy_and_hold_performance, self.buy_and_hold_transaction_cost, self.buy_and_hold_tax = self._compute_performance(data, trade_dates=trade_dates, trade_cost=trade_cost, spread=spread, saving_plan=saving_plan, saving_plan_period=saving_plan_period, tax_rate=tax_rate, tax_allowance=tax_allowance)
        ...
        for i in range(self.time-1):
            ...
            if swing_performance[-1] <= 0: # If we are broke
                ...
                if saving_plan != 0 and i % saving_plan_period == 0 and i != 0: # If we have a saving plan
                    ...
                    value_at_last_trade += (saving_plan-trade_cost[1]) * (1-spread)
    ...
    def print_results(self, accuracy=2, *args, **kwargs):
        ...
        print("Initial investment: ", f"{self.initial_investment:,.2f}")
        print("Total investment: ", f"{self.total_investment:,.2f}")
        print()
        ...
        print(f"    Absolute: {absoulte_performance:,.2f}, Relative: {relative_performance:,.2f}")
        print(f"    Yearly performance: {yearly_return:,.2f}")
        print()
        ...
        print(f"    Absolute: {absoulte_performance:,.2f}, Relative: {relative_performance:,.2f}")
        print(f"    Yearly performance: {yearly_return:,.2f}, Internal rate of return: {internal_rate:,.2f}")
...
class MonteCarloSimulation:
    ...
    def print_results(self, accuracy=2, *args, **kwargs):
        ...
        print("Initial invetment: ", f"{initial_investment:,.2f}")
        print("Total invetment: ", f"{total_investment:,.2f}")
        print()
        ...
        print(f"Index performance:")
        print(f"  Overall return: {overall_return:,.2f} +/- {overall_std:,.2f} (Median: {overall_median:,.2f})")
        print(f"  Relative performance: {relative_performance:,.2f} +/- {relative_std:,.2f} (Median: {relative_median:,.2f})")
        print(f"  Yearly performance: {yearly_return:,.2f} +/- {yearly_std:,.2f} (Median: {yearly_median:,.2f})")
        print()
        ...
        print(f"  Relative performance: {relative_performance:,.2f} +/- {relative_std:,.2f} (Median: {relative_median:,.2f})")
        print(f"  Yearly performance: {yearly_return:,.2f} +/- {yearly_std:,.2f} (Median: {yearly_median:,.2f})")
        print(f"  Internal rate of return: {internal_rate:,.2f} +/- {internal_rate_std:,.2f} (Median: {internal_rate_median:,.2f})")
        print(f"  Taxes: {taxes:,.2f} +/- {taxes_std:,.2f} (Median: {taxes_median:,.2f})")
        print(f"  Transaction cost: {transaction_cost:,.2f} +/- {transaction_cost_std:,.2f} (Median: {transaction_cost_median:,.2f})")
        print()
